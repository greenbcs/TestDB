...
if($row1==0){
    $result=mysqli_query($conn,"select SW from pmts_storage where NO ='$row1'");
}else{
    $result=mysqli_query($conn,"select SW from pmts_storage where NO=(select max(NO) from pmts_storage)");
}
